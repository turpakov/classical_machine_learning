...
def perceptron(input_data, w):
    ...
    for i in range(len(classes) - 1):
        ...
        while iter < count:
            ...
            iter += 1
        all_lines__.append(calc_line(x_for_print, record))
        all_w.append(record)
        ...
        if len(x_for_print[0]) == 3:
            ...
            boss_lines.append(all_lines[-1])

            for j, line in enumerate(all_lines):
                plt.clf()
                plt.xlim(min([x[0] for x in x_for_print])-1, max([x[0] for x in x_for_print])+1)
                plt.ylim(min([x[1] for x in x_for_print])-1, max([x[1] for x in x_for_print])+1)
                if i >= 1:
                    for k in range(i):
                        plt.plot([boss_lines[k][0], boss_lines[k][1]], [boss_lines[k][2], boss_lines[k][3]], c=ALL_COLORS[k])
                for l in range(len(classes)):
                    for x in x_for_print:
                        if x[-1] == classes[l]:
                            plt.scatter(x[0], x[1], c=ALL_COLORS[l], marker=ALL_MARKERS[l])
                plt.plot([line[0], line[1]], [line[2], line[3]], c=ALL_COLORS[i])
                plt.pause(0.1)
    if len(x_for_print[0]) == 3:
        print(f"Классы: {classes}")
        ...
        for i, w in enumerate(all_w):
            ...
            print(s[:-8])
    if len(x_for_print[0]) == 4:
        show_result(all_lines__, x_for_print, classes, all_w)
    plt.show()




input_data = init_data("test_3d.txt")

perceptron(input_data, np.array([1 for i in range(len(input_data[0]))]).T)
